...
function MessageDetail() {
  ...
  const handleChange = (event) => {
    ...
    // const { name, value,images, files } = event.target;
    // setnewMessage(prev => ({
    //   ...prev,
    //   [name]: name === 'image' ? images[0] : value,
  ...
  const SendMessage = () => {
    ...
    // if (newMessage.file) {
    //   formdata.append("message_file", newMessage.file);
    // }
    // if (newMessage.image) {
    //   formdata.append("message_image", newMessage.image);
    // }
    try {
    ...
    <div>
      <main className="content" style={{ marginTop: "150px" }}>
        <div className="container p-0">
          <h1 className="h3 mb-3" style={{  color: "rgb(24, 92, 130) " }}>Messages</h1>
          <div className="card">
                ...
                {messages.map((message) =>
                  ...
                  >
                    ...
                    <div className="d-flex align-items-start">
                      ...
                      {message.sender.id === user_id ? (message.reciever_profile.full_name || message.reciever.username) : message.sender.username}
                        <div className="small">
              ...
              <div className="col-12 col-lg-7 col-xl-9">
                ...
                <div className="position-relative">
                  <div className="chat-messages p-4">
                    {message.map((message) =>
                      <React.Fragment key={message.id || `${message.sender.id}-${message.date}`}>
                        {message.sender.id !== user_id &&
                          <div className="chat-message-left pb-4"  >
                            ...
                            <div className="flex-shrink-1 bg-light rounded py-2 px-3 mr-3">
                              <div className="font-weight-bold mb-1">{user.username}</div>
                              {message.message}
                              <br />
                ...
                <div className="flex-grow-0 py-3 px-4 border-top">
                  <div className="input-group">
                    ...
                    name="message_image" 
                    onChange={handleChange}  key={message.id ? message.id : `${message.sender.id}-${message.date}`}
                  />
                  <input
                    type="file"
                    className="form-control-file"
                    name="message_file" // حقل الملف
                    onChange={handleChange} // استخدام نفس الدالة handleChange
                  /> */}
